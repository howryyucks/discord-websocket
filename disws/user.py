...
class BaseUser(UserTag):
    ...
    def _update(self, data: User) -> None:
        ...
        self.avatar: Optional[str] = (
            get_avatar_url(self.id, data["avatar"])
...
class Me(BaseUser):
    ...
    def _full_update(self, data: User) -> None:
        ...
        self.premium_type: Optional[str] = (
            get_flag(data["premium_type"]) if data.get("premium_type", None) else None
        )
...
class Member(BaseUser):
    ...
    def _guild_update(
        ...
        if data is not None:
            self.guild: Optional[disws.guild.Guild] = (
                disws.guild.Guild(data["guild"]) if data.get("guild", None) else None
            )
            self.nick: Optional[str] = data.get("nick", None)
            self.guild_avatar: Optional[str] = (
                guild_member_avatar_url(self.guild.id, self.id, data["avatar"])
                if data.get("avatar", None) and data.get("guild", None)
                else None
            )
            self.roles: List[Role] = (
                self._get_roles(data.get("roles")) if data.get("roles", None) else []
            )
            self.premium_since: Optional[str] = (
                from_iso_format_to_humanly(data["premium_since"])
            ...
            self.timeout: Optional[str] = (
                from_iso_format_to_humanly(data["communication_disabled_until"])
